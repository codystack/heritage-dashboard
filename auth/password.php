...
if(isset($_POST['password_change_btn'])) {
    ...
    $sql=mysqli_query($conn,"SELECT * FROM tbl_admin where password='$password' && id='".$_SESSION['id']."'");
    $num=mysqli_fetch_array($sql);
    if($num>0)
    {
        $conn=mysqli_query($conn,"UPDATE tbl_admin SET password='$newPassword' where id='".$_SESSION['id']."'");
